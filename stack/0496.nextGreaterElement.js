...
var nextGreaterElement = function (nums1, nums2) {
    ...
    for (let i = nums2.length - 1; i >= 0; i--) {
        let before = i + 1;
        if (before >= nums2.length) {
            map.set(nums2[i], -1)
        } else {
            if (nums2[before] > nums2[i]) {
                map.set(nums2[i], nums2[before])
            } else {
                map.set(nums2[i], getNextFunction(map, nums2[before], nums2[i]))
            }
        }
    }
...
function getNextFunction(map, before, current) {
    while (before !== -1) {
        if (before > current) {
            return before
        } else {
